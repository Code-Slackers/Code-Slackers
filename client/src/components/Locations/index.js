      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
        {locations &&
          locations.map((location) => (
            <div key={location._id} id={location._id} className="rounded-lg shadow-md cursor-pointer" onClick={viewLocationHandler}>
              <div className="relative">
                ...
                <div className="absolute inset-x-0 bottom-0 px-4 py-2">
                  <h4 className="text-xl font-semibold text-white">{location.city}, {location.state}</h4>
                </div>
              </div>
            </div>
          ))}
